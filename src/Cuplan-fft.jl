...
"""
    CuPlanFFT{T,Tf,Ti}( ; nx::Int, nz::Int, px::Int, pz::Int, T::DataType)
...
"""
struct CuPlanFFT{T, A1, A2, C2, Tf, Ti}
    nx::Int
    nz::Int
    px::Int
    pz::Int
...
"""
    show(io::IO, ::MIME"text/plain", plan::CuPlanFFT)
"""
function Base.show(io::IO, ::MIME"text/plain", plan::CuPlanFFT{T, A1, A2, C2, Tf, Ti}) where {T, A1, A2, C2, Tf, Ti}
    t = typeof(plan)
    println(io, t)
...
"""
    show(io::IO, mime::MIME"text/plain", vp::Vector{<:CuPlanFFT})
"""
function Base.show(io::IO, mime::MIME"text/plain", vp::Vector{<: CuPlanFFT})
    t = typeof(vp)
...
"""
    sizeof(::CuPlanFFT)
Show size in bytes of `CuPlanFFT` object.
"""
function Base.sizeof(ob::T) where {T <: CuPlanFFT}
