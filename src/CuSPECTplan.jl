...
- `planrot` Vector of struct `CuPlanRotate`
- `planpsf` Vector of struct `CuPlanFFT`
...
struct CuSPECTplan{T,A2,A3,A4}
    ...
    dy::T
    planrot::CuPlanRotate # todo: make it concrete?
    planpsf::CuPlanFFT

    """
        CuSPECTplan(mumap, psfs, dy; T, viewangle)
    """
    function CuSPECTplan(
        ...
        planrot = CuPlanRotate(nx; T)

        planpsf = CuPlanFFT(; nx, nz, px, pz, T)
...
"""
function Cugen_attenuation!(plan::CuSPECTplan, y::Int)
    Cuscale3dj!(plan.exp_mumapr, plan.mumapr, y, -0.5)
    for j = 1:y
        Cuplus3dj!(plan.exp_mumapr, plan.mumapr, j)
    end
...
"""
    show(io::IO, ::MIME"text/plain", plan::CuSPECTplan)
"""
function Base.show(io::IO, ::MIME"text/plain", plan::CuSPECTplan{T,A2,A3,A4}) where {T,A2,A3,A4}
    t = typeof(plan)
    println(io, t)
...
"""
    sizeof(::CuSPECTplan)
Show size in bytes of `CuSPECTplan` object.
"""
function Base.sizeof(ob::T) where {T <: Union{CuSPECTplan}}
