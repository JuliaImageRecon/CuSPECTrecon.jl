...
"""
    Cupsf_gauss( ; ny, px, pz, fwhm_start, fwhm_end, fwhm, fwhm_x, fwhm_z, T)
...
function Cupsf_gauss( ;
    ...
    for iy in 1:ny # depth-dependent blur
        psf[:,:,iy] = Cupsf_gauss(rx, fwhm_x[iy]) * Cupsf_gauss(rz, fwhm_z[iy])'
    end
    return psf
end

function Cupsf_gauss(r::AbstractVector, fwhm::Real)
   if fwhm == 0
